...
.then(raw => {
    console.log('raw:', raw)
    const data = updateData(raw);
    console.log('data:',data);
    const chart = updateViz(data);
...
function updateData (raw){  
  ...
  const pc=  [...pc_root, ...pc_tab, ...pc_group, ...pc_link];
...
function updateViz(data) {
    ...
    const gNode = svgCanvas.append("g")
        .attr("cursor", "pointer")
    ...
    function update(source) {
      ...
      const transition = svgCanvas.transition()
          .duration(duration)
          .attr("viewBox", [-margin.left, left.x - CANVAS.TOP, width, height])
...
function  diagonal (){ d3.linkHorizontal().x(d => d.y).y(d => d.x)  }
function tree(){ d3.tree().size([dx, dy])} //size() was: nodeSize()
